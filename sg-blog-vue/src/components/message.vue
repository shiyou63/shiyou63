      ...
      <form class=""  >
        ...
        <el-row class="tmsg-r-info">
          <el-col :span="24" class="info-submit">
            <p
              class="tcolors-bg"
              :class="{ 'disabled': isSubmitting || countdown > 0 }"
              @click="sendMsg"
            >
              {{ countdown > 0 ? `请等待 ${countdown} 秒` : sendTip }}
            </p>
          </el-col>
        </el-row>
      </form>
      ...
      <div class="tmsg-commentshow">
        <ul class="tmsg-commentlist">
          <li class="tmsg-c-item" v-for="(item,index) in commentList" :key="'common'+index">
            <article class="">
              <header>
                <img  :src="item.avatar || $store.state.defaultAvatar"  :onerror="$store.state.errorImg">
                <div class="i-name">
                  {{item.username}}
                </div>
                <!-- <div class="i-class">
              ...
              <section>
                ...
                <div v-if="haslogin" class="tmsg-actions">
                  <!-- 回复按钮 -->
                  ...
                  <!-- 删除按钮（条件判断修正） -->
                  <el-popconfirm
                    v-if="$store.state.user && ($store.state.user.role === 'admin' || item.createBy === $store.state.user.id)"
                    title="确定要删除这条评论吗？"
            ...
            <ul v-show="item.children" class="tmsg-commentlist" style="padding-left:60px;">
              <li class="tmsg-c-item" v-for="(citem,cindex) in item.children" :key="'citem'+cindex">
                <article class="">
                  <header>
                    <img :src="citem.avatar || $store.state.defaultAvatar":onerror="$store.state.errorImg">
                    <div class="i-name">
                  ...
                  <section>
                    ...
                    <el-popconfirm
                      v-if="$store.state.user && ($store.state.user.role === 'admin' || citem.createBy === $store.state.user.id)"
                      title="确定要删除这条评论吗？"
                      @confirm="handleDeleteComment(citem.id)"
                    >
                      <template #reference>
                        <el-button
                          type="danger"
...
export default {
  ...
  computed: {
    ...
    getButtonText() {
      ...
      if (this.isSubmitting) return '发送中...';
      return this.sendTip;
    }
  },
  methods: {
    async handleDeleteComment(commentId) {
      try {
        ...
        this.commentList = removeComment(this.commentList)
      } catch (error) {
    ...
    sendMsg:function(){//留言
      var that = this;
      if (that.isSubmitting || that.countdown > 0) return;
      if(that.textarea){
        that.isSubmitting = true; // 锁定提交状态
        that.sendTip = '咻~~';
        sendComment(that.type,that.aid,that.rootId,that.toCommentId,that.toCommentUserId,that.textarea)
          .then((response)=>{
            // 成功处理...
            that.textarea = '';
            that.rootId = -1;
          ...
          .catch((error) => {
            // 新增：频率限制错误处理（假设错误码为50010）
            if (error.response && error.response.data && error.response.data.code === 50010) {
              that.startCountdown(60); // 60秒冷却
              that.$message.error(error.response.data.msg || '操作过于频繁');
            } else {
              // 其他错误处理...
            }
          })
